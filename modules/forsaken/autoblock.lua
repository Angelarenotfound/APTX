...
blockdelay = 0, looseFacing = true, detectionRange = 18local AutoBlock = {}
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local TweenService = game:GetService("TweenService")
local RunService = game:GetService("RunService")
local Players = game:GetService("Players")
...
local function fireGuiBlock() if not testRemote then return end testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Block\"")}) end
local function fireGuiPunch() if not testRemote then return end testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Punch\"")}) end
local function fireGuiCharge() if not testRemote then return end testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Charge\"")}) end
local function fireGuiClone() if not testRemote then return end testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Clone\"")}) end
local function playCustomAnim(animId, isPunch)
local Humanoid = getHumanoid()
if not Humanoid or not animId or animId == "" then return end
local now = tick()
...
local plr = Players:GetPlayerFromCharacter(model)
if not plr or plr == lp then return end
if State.facingCheckEnabled and not isFacing(myRoot, hrp) then return end
task.wait(State.blockdelay)
if mode == "Block" then
if cachedCooldown and cachedCooldown.Text == "" then fireGuiBlock() else return end
if State.doubleblocktech == true then fireGuiPunch() end
elseif mode == "Charge" then
if cachedChargeBtn and cachedChargeBtn:FindFirstChild("CooldownTime") and cachedChargeBtn.CooldownTime.Text == "" then fireGuiCharge() startChargeAimUntilChargeEnds(0.4) else return end
elseif mode == "Clone" then
if cachedCloneBtn and cachedCloneBtn:FindFirstChild("CooldownTime") and cachedCloneBtn.CooldownTime.Text == "" then fireGuiClone() startChargeAimUntilChargeEnds(0.4) else return end
end
lastLocalBlockTime = now
soundBlockedUntil[sound] = now + AUDIO_SOUND_THROTTLE
end
...
if blocked then
if not (State.facingCheckEnabled and not isFacing(myRoot, hrp)) then
if State.autoblocktype == "Block" then fireGuiBlock()
elseif State.autoblocktype == "Charge" then fireGuiCharge()
elseif State.autoblocktype == "7n7 Clone" then fireGuiClone() end
soundBlockedUntil[sound] = t + 1.2
end
break
end
...
function AutoBlock.Init()
if not (KillersFolder and testRemote) then refsReady.Event:Wait() end
lp.CharacterAdded:Connect(function(char) task.wait(0.5) refreshAnimator() task.delay(0.5, refreshUIRefs) end)
refreshUIRefs()
if cachedPlayerGui then
...
if State.autoBlockOn and (hrp.Position - myRoot.Position).Magnitude <= State.detectionRange then
if isFacing(myRoot, hrp) then
if cooldown and cooldown.Text == "" then fireGuiBlock() end
if State.doubleblocktech == true and charges and charges.Text == "1" then fireGuiPunch() end
end
end
end
end
