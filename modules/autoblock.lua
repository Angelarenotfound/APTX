...
local function fireGuiBlock() testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Block\"")}) end
local function fireGuiPunch() testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Punch\"")}) end
local function fireGuiCharge() testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Charge\"")}) end
local function fireGuiClone() testRemote:FireServer("UseActorAbility", {buffer.fromstring("\"Clone\"")}) end
...
function AutoBlock.Init()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if cachedAnimator then
            ...
            if ok and tracks then
                for _, track in ipairs(tracks) do
                    local animId = pcall(function() return tostring(track.Animation and track.Animation.AnimationId or ""):match("%d+") end)
